...
function loginpop(id, rep, docu, lGe) {
    ...
    if (lg != null) {
        ...
        ht.find('.documentation').off().click(function() {
            var aaaa = 'cp.nd?id=' + id + '&cmd=documentation&token=' + token + '&documentation=' + JSON.stringify({ documentation: ht.find('.documentationc').is(":checked"), loginG: ht.find('.loginG').is(":checked") });
            $.get(aaaa,
                function(d) {
                    var data = JSON.parse(d)
                    alert(data.msg)
        ...
        if (names != null) {
            $.each(names, function(i, e) {
                ...
                if (e[0] == key) { name = e[1] } else
                    okeys.splice(okeys.indexOf(e[0]), 1);
...
function sendtheme(id, onsend) {
    ...
    $(e).on('change', function() {
        ...
        xx = $.ajax({
            ...
            timeout: 0,
            url: 'theme?cmd=roomsTheme&f=theme&t=' + ty + '&token=' + token,
            //      url: 'pic?secid=room&fo=pic',
            type: 'POST',
            data: formData,
            cache: false,
...
function logins() {
    ...
    $.get(urlg, function(d) {
        ...
        $.each(data, function(i, e) {
            if (e.power != 'Hide') {
                var btn = "<a class='btn btn-primary fa fa-gear' onclick='loginpop(\"" + e.idreg + "\"," + e.rep + "," + e.documentationc + "," + e.loginG + ");'></a>";
...
function powers() {
    $.get('cp.nd?cmd=powers&token=' + token, function(d) {
        var data = JSON.parse(d);
        if (data.err === true) {
            $('#powers').text('لا تملك صلاحيات');
            return;
        }
        var us = { rank: 999, name: "تلقائي", ico: "", itar: "", them: "", kick: 9999, delbc: false, alert: false, mynick: false, unick: false, ban: false, publicmsg: 9999, forcepm: false, loveu: false, roomowner: false, createroom: false, rooms: 9999, edituser: false, meiut: false, ulike: false, flter: false, subs: false, shrt: false, msgs: false, bootedit: false, setpower: false, upgrades: 9999, history: false, cp: false, stealth: false, owner: false, grupes: false, delpic: false, delmsg: false }
        if (data.length <= 0)
            data.push(us)
        powersarr = data;
...
function bansIs() {
    $.get('cp.nd?cmd=bansU&token=' + token, function(d) {
        ...
        $.each(data, function(i, e) {
            ...
            // <!-- var btn2="<a style='"+(e.decoderDans?'background: #258c05;border-color: #258c05;':'')+"' class='dec btn btn-danger fa fa-times' onclick='decoderDans({id:\""+e.id+"\",decoderDans:"+ e.decoderDans+"});'>"+adfse+"</a></a>"; -->
            ...
            // <!-- tableAdd(h,['*', e.device_band || e.ip_band || e.country_band,new Date(Number(e.date)).toDateString()],e.id,btn);  -->
        });
...
function roomstheme(id) {
    sendtheme('.' + id, function(file) {
        var f = JSON.parse(file);
        if (f && f.error) return alert(d.msg)
        $.get('cp.nd?cmd=roomstheme&token=' + token + '&theme=' + f.msg + '&id=' + id, function(d) {
            var data = JSON.parse(d);
            if (data.err === true) {
                alert(data.msg)
                return;
            }
            $('#rooms>.tablesorter>tbody>#' + id + ' img.rtheme').attr('src', '/' + data.msg)
        });
    });
}
...
function roomsthemedel(id) {
    var sdf = $('#rooms>.tablesorter>tbody>#' + id + ' img.rtheme')
    if (sdf.attr('src') == 'room.png' || sdf.attr('src') == '/room.png') return alert('الغرفه تحتوي على الصورة الافتراضية بالفعل ');
    $.get('cp.nd?cmd=roomsthemedel&token=' + token + '&theme=room.png&id=' + id, function(d) {
        var data = JSON.parse(d);
        if (data.err === true) {
            alert(data.msg)
            return;
...
<tbody class='rrrr'>
<tr >
<td style="min-width: 130px;"><img style="max-width:64px;max-height:64px;min-height: 30px;" class="fl r` + e.id + `" src="` + '/' + e.pic + `"><a style="margin-bottom: 2px;" class='btn btn-info fa fa-photo fr ' onclick='roomspic("` + e.id + `");'>تغير</a><br><a class='fr btn btn-danger fa fa-times'  onclick='roomspicdel("` + e.id + `");'>حذف</a></td>
<td style="min-width: 130px;"><img style="max-width:64px;max-height:64px;min-height: 30px;" class="fl rtheme ` + e.id + `" src="` + '/' + e.theme + `"><a style="margin-bottom: 2px;" class='btn btn-info fa fa-photo fr ' onclick='roomstheme("` + e.id + `");'>تغير</a><br><a class='fr btn btn-danger fa fa-times'  onclick='roomsthemedel("` + e.id + `");'>حذف</a></td>
<td><a ` + (e.needpass ? '' : 'disabled') + ` class='passDelete fr btn btn-danger fa fa-times ' onclick='roomdelpass("` + e.id + `");'>حذف</a></td>
<td><a class=' btn btn-danger fa fa-times' onclick='roomsdel("` + e.id + `");'>حذف</a></td>
<td><a ` + (data.data != e.id ? '' : 'disabled') + ` class='chekedroom btn btn-info fa fa-check' onclick='roomsver("` + e.id + `");'>تحديد الغرفة</a></td>
		</tr>
		</tbody>
		</table></div>
`
...
function hostin() {
    ...
    $.get('cp.nd?cmd=coom&token=' + token, function(d) {
        var e = JSON.parse(d);
        if (e.msg) {
            $('#cooment').text('(مفعل)');
            $('#cooment').css('color', 'green');
        } else {
            $('#cooment').text('(غير مفعل)');
            $('#cooment').css('color', 'red');
        };
    });
...
function sett(dom) {
    ...
    $.get('cp.nd?cmd=owner&domin=' + dom + '&token=' + token + '&rnd=' + new Date().getTime().toString(), function(d) {
        ...
        var pitar = $('.p-sitar');
        pitar.children().remove();
        $.each(JSON.parse(data.sitar), function(i, e) {
            var ht = $('<div style="display:inline-block;padding:2px;margin:2px;margin-top:2px;" class="border"><img style="max-width:24px;max-height:24px;"><a style="margin-left: 4px;padding:4px;" onclick="del_ico(this);" class="btn btn-danger fa fa-times">.</a></div>')
            ht.find('img').attr('src', '/sitar/' + e);
            ht.find('a').attr('pid', '/sitar/' + e);
            pitar.append(ht);
        });
        var pthem = $('.p-theme');
        pthem.children().remove();
        $.each(JSON.parse(data.theme), function(i, e) {
            var ht = $('<div style="display:inline-block;padding:2px;margin:2px;margin-top:2px;" class="border"><img style="max-width:24px;max-height:24px;"><a style="margin-left: 4px;padding:4px;" onclick="del_ico(this);" class="btn btn-danger fa fa-times">.</a></div>')
            ht.find('img').attr('src', '/theme/' + e);
            ht.find('a').attr('pid', '/theme/' + e);
            pthem.append(ht);
        });
        pico = $('.p-dro3');
        pico.children().remove();
...
function s_sitar(fn) {
    ...
    $.get('cp.nd?cmd=addico&token=' + token + '&pid=' + fn + '&tar=sitar', function(data) {
        var d = JSON.parse(data)
        if (d) {
            if (d.error) {
                alert('فشل رفع الصوره')
...
function s_theme(fn) {
    ...
    $.get('cp.nd?cmd=addico&token=' + token + '&pid=' + fn + '&tar=theme', function(data) {
        var d = JSON.parse(data)
        if (d) {
            if (d.error) {
                alert('فشل رفع الصوره')
...
function sendfileaDis() {
    ...
    var setthead = $("#sett_head").val()

    var settkeywords = $('#sett_keywords').val();
    var name = $("#sett_name").val(),
        title = $("#sett_title").val();
    var d = { name: name, title: title, settdescription: settdescription, settscr: settscr, setthead: setthead, settkeywords: settkeywords };
